...
function getItem() {
    ...
    function handleAddComment() {
            ...
            firebase.db.collection('forum').doc(itemId).get().then(doc => {
                if(doc.exists) {
                    ...
                    firebase.db.collection('forum').doc(itemId).update({comments: updatedComments})
                    setItem(prevState => ({
                        ...prevState, 
...
<div>PYTANIE: {item.description}</div>
        <div>TYTUŁ: {item.title}</div>
             ...
             {item.comments.map((comment, index) => {
                    return <div >{comment.text} Dodano: {formatDistanceToNow(comment.creted)}</div>
                    })}
                </div>
